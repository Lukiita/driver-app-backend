...
export class RideRepositoryDatabase implements RideRepository {
  ...
  async getActiveRidesByPassengerId(passengerId: string): Promise<Ride[]> {
    ...
    return activeRides.map((ride: any) => Ride.restore(ride.ride_id, ride.passenger_id, parseFloat(ride.from_lat), parseFloat(ride.from_long), parseFloat(ride.to_lat), parseFloat(ride.to_long), ride.status, ride.date, parseFloat(ride.last_lat), parseFloat(ride.last_long), parseFloat(ride.distance), parseFloat(ride.fare), ride.driver_id));
  }
